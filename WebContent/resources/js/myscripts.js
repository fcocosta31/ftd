...
function enviarEmailPedidoOrcam(nome, idftd, formapgto, transportadora, observ){
	preloadActive();

	$.get('srl',{acao:"sendmailpedidoorcam", txtnome:nome, txtidempresa:idftd,
		txtfonecontato:formapgto, 
		txttransportadora:transportadora, txtobs: observ}, function(msg){
		preloadDeActive();
	...
	$.get('srl',{acao:"listaritensatendidos", txtcodigo:codigo, txtidpedido:idpedido},
			...
			$.each(dados, function(i, item){
				...
				+"<input type='number' min='0' max='"+item.qtdatendida+"' value='"+item.qtdatendida+"'" +
						" name='"+codigo+"item"+count+"' style='width: 60px'></td>" +
				...
				"<input type='submit' value='Enviar'></td></tr></form></table></td></tr>";
			});
			tabela += "</tbody></table>";
