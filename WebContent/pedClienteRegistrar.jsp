<script type="text/javascript">
	...
	function openModalAlterarNomeCliente(){
		$("#modalAlterarNomeCliente").modal('open');
	};
		
	$(document).on('click', '#btnConfirmaAlterarNomeCliente', function(e){
		var idempresa = $("#txtidnomecliente").val();
		sendAjaxAlterarNomeCliente(idempresa);
	});
	...
	$(document).on('click', '#btnDialogSalvarPedido', function(e){
		...
		if(idempresa == ''){
			mbox.alert("Selecione um cliente no campo de pesquisa!",function(){});
		}else{
	...
	function checkInput(textbox) {
		 ...
		 var atendido = document.getElementById(textbox).value;
		 var pedido = document.getElementById(textbox+'pdo').value;
		 if (atendido > pedido){
			 document.getElementById(textbox).value = pedido;
		 } 
	}
					...
					<form id="formitenspedido" action="srl" method="post">
						...
						<span class="item-topico-left">
							...
							<br>
								...
								<c:choose>
									<c:when
										test="${empty usuariologado or usuariologado.cargo gt 2}">
											<span class="item-topico-left">
												Cliente: ${pedcliente.cliente.codigoftd} - ${pedcliente.cliente.razaosocial}
												<br>
											</span>
									</c:when>
									<c:when
										test="${usuariologado.cargo eq 1 or usuariologado.cargo eq 2}">
												<span class="item-topico-left">
													Cod.Cliente:
												...
												<input type="text" id="txtdescricaoempresa" ${readmode}
													...
													<c:if test="${readmode eq 'readonly' or empty pedcliente.cliente.codigoftd}">
														<button type="button"
															onclick="openModalAlterarNomeCliente()" 
						...
						<c:if test="${empty usuariologado or usuariologado.cargo gt 2}">
						...
						<!-- INÍCIO DOS BUTTONS -->
						<c:if
							test="${!empty usuariologado and usuariologado.cargo eq 1 or usuariologado.cargo eq 2}">

								<div class="noPrint right-align col l6 m6 s12 input-group div-title-bottom">

								<c:if test="${fn:containsIgnoreCase(pedcliente.situacao, 'AGUARDANDO')}">
						...
						<!-- FIM DOS BUTTONS -->
							...
							<div class="row">
								<c:if test="${fn:containsIgnoreCase(pedcliente.situacao, 'ATENDIDO')}">
								<div class="noPrint right-align col l6 m6 s12 input-group div-title-bottom">
									<h5>Último(s) atendimento(s) </h5>
									<c:forEach var="d" items="${historico}">
											<a href="srl?acao=detalharhistoricopedcliente&idpedido=${pedcliente.idpedido}&txtdata=${d}">
											<span>[<fmt:formatDate	value="${d}" pattern="dd/MM/yyyy" />] </span>
											</a>										
									</c:forEach>
								</div>
								</c:if>
						...
						<c:if test="${fn:containsIgnoreCase(pedcliente.situacao, 'ATENDIDO')}">
							<c:set var="inputreadmode" value="readonly" />
						</c:if>
									...
									<c:forEach var="pd" items="${pedcliente.itens}">
										...
										<tr class="${bgcor}">
											...
											<td><input type="number" min="1" tabindex="-1"
												...
												ondblclick="alterarQtdeItemPedido('${pd.item.codigo}', '${pedcliente.idpedido}', '${qtde}')">
											</td>
											<td><fmt:formatNumber
													value="${pd.qtdpedida * pd.item.preco}" type="number"
													pattern="#,#00.00#" /></td>
											<td><input type="number" min="0" max="${pd.qtdpedida}" ${inputreadmode} class="input-values-pedcliente"
												name="${pd.item.codigo}atd" value="${pd.qtdatendida}" id="${pd.item.codigo}"
												ondblclick="listItensAtendidos('${pd.item.codigo}', '${pedcliente.idpedido}')"
												>
											</td>																																								
						...
						<input type="hidden" value="pedidoregistrar" name="acao"
							id="btnacao">
						<input type="hidden" value="0" name="tiporecord">
					</form>
				</div>
	...
	<div class="modal noPrint" id="modalAlterarNomeCliente">
		<div class="center-align div-title-modal">
		...
		<div class="modal-content">
			<input type="text" id="txtnomecliente"
				placeholder="Digite o nome da empresa..."
				name="txtnomecliente" autocomplete="off"
				data-activates="singleDropdownNomeCliente" data-beloworigin="true">
			<p style="font-size: 8pt"
				id="txtnomecliente-description"></p>				
			<input type="hidden" id="txtidnomecliente">
			<ul id="singleDropdownNomeCliente" constrainWidth="false" class="dropdown-content ac-dropdown col s12"></ul>																		
		</div>
		<div class="modal-footer">
			<a href="#!" id="btnConfirmaAlterarNomeCliente" 
